...
void accessSeq(char *filename){
    ...
    unsigned long pagefcount = 0;
    //unsigned long pagef[ULONG_MAX];
    unsigned long *memAccesses, *memWrite;
    
    char *outputFile = "output-part2";
   ...
   // }
    ...
    for(i = 0; i < filesize/sizeof(unsigned long);i++){
        ...
        unsigned long virtAddress = virtualMap(memAccesses[i]);

        unsigned long currOffset = offset(memAccesses[i]);

        unsigned long physPage = getPhysPage(virtAddress);
        
        if(physPage == NOT_FOUND){
            pagefcount++;
           // pagef[virtAddress]++;
            physPage = getFreePage();
        }
        ...
        assignVirtPage(physPage, virtAddress);
